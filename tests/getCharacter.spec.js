...
- Se o parâmetro não estiver na a
...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    expect(getCharacter('arya')).toMatchObject({name: 'Arya Stark',class: 'Rogue',
      phrases: ['Not today', 'A girl has no name.']
    })
    ...
    const expected = { name: 'Arya Stark', class: 'Rogue',
      phrases: ['Not today', 'A girl has no name.']
    };
    const capitalLetter = getCharacter('Arya');
    const lowerCase = getCharacter('arya');
    expect(capitalLetter).toEqual(expected);
    expect(lowerCase).toEqual(expected)
    // 6. Teste se ao passar um nome que não está na tabela, a função retorna undefined.
    const personagem = getCharacter('anyName')
    expect(personagem).toBeUndefined();
  });
});
